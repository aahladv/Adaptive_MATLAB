...
close all
global controlinput wHat t1 t2 u wHatdummy thetaHatdummy
...
figure(7)
time = 0:tf/length(thetaHatdummy):tf;
time(end) = [];
plot(time,thetaHatdummy(1,:))
hold on 
plot(time,thetaHatdummy(2,:))
title('parameters estimate error');
xlabel('Time(s)')
ylabel('Torque(Nm)')

function [XDot] = twoLinkdynamics(t,X,theta)
global u controlinput t1 t2 wHatdummy wHat thetaHatdummy
t;
...
wHat         = horzcat(wHatdummy,wHat);

thetaHatdummy         = horzcat(thetaHatdummy,theta - thetaHat);
